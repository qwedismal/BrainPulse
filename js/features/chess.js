...
// Набор простых тактических задач "мат в 1 ход"
// Формат: FEN-позиция + правильный ход
const PUZZLES = [
  {
    fen: 'r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/5Q2/PPPP1PPP/RNB1K1NR w KQkq - 0 3',
    solution: ['f3', 'f7'],
    description: 'Ферзь бьёт пешку f7 — мат!'
  },
  {
    fen: '6k1/5ppp/8/8/8/8/5PPP/4R1K1 w - - 0 1',
    solution: ['e1', 'e8'],
    description: 'Ладья ставит мат на 8-й линии'
  },
  {
    fen: 'r3k2r/ppp2ppp/2n5/3p4/3P4/2N5/PPP2PPP/R3K2R w KQkq - 0 1',
    solution: ['e1', 'g1'],
    description: 'Короткая рокировка для безопасности'
  },
  {
    fen: 'r1bq1rk1/pppp1ppp/2n2n2/2b1p3/2B1P3/3P1N2/PPP2PPP/RNBQ1RK1 w - - 0 1',
    solution: ['f3', 'e5'],
    description: 'Конь атакует центральную пешку'
  },
  {
  ...
  },
  {
    ...
    solution: ['d1', 'h5'],
    description: 'Ферзь атакует слабую пешку h7'
  },
  {
    fen: '4k3/8/8/8/8/8/4Q3/4K3 w - - 0 1',
    solution: ['e2', 'e8'],
    description: 'Ферзь ставит мат через шах'
  },
  {
    fen: 'r1b1k2r/ppppnppp/2n5/3pP3/3P4/2N5/PPP2PPP/R1BQKB1R w KQkq - 0 1',
    solution: ['g1', 'f3'],
    description: 'Развитие коня на f3'
  },
  {
...
function isValidMove(board, fromR, fromC, toR, toC) {
  // Упрощённая проверка — для целей задачи достаточно базовой валидации
  ...
  if (target && (piece === target.toUpperCase()) === (target === target.toUpperCase())) {
    // Обе фигуры одного цвета (заглавные = белые)
...
function renderBoard(puzzle) {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      const isHighlight = STATE.highlights && STATE.highlights.includes(sqName);
      const isCapture = STATE.captureSquares && STATE.captureSquares.includes(sqName);
      
      let cls = 'chess-square ' + (isLight ? 'light' : 'dark');
      if (isSelected) cls += ' selected';
      if (isHighlight) cls += ' highlight';
      if (isCapture) cls += ' possible-capture';
      else if (isHighlight) cls += ' possible-move';
      
      html += `<div class="${cls}" data-sq="${sqName}" data-r="${r}" data-c="${c}">
        ${piece ? `<span style="color:${piece === piece.toUpperCase() ? '#fff' : '#000'};text-shadow:${piece === piece.toUpperCase() ? '0 1px 2px rgba(0,0,0,0.8)' : '0 1px 2px rgba(255,255,255,0.4)'}">${PIECES[piece]}</span>` : ''}
      </div>`;
    }
  }
  html += '</div>';
  ...
  document.querySelectorAll('.chess-square').forEach(sq => {
    sq.addEventListener('click', () => onSquareClick(sq.dataset.sq, sq.dataset.r, sq.dataset.c));
  });
}
...
function onSquareClick(sqName, r, c) {
  ...
  if (!STATE.selectedSquare) {
    ...
    if (sqName === targetSq || sqName.toUpperCase() === targetSq.toUpperCase()) {
      // Правильный ход
      Snd.ok();
      Utils.flash('green');
      STATE.score++;
      ...
      Snd.err();
      Utils.flash('red');
      document.getElementById('problem-area').style.animation = 'shake 0.3s';
      setTimeout(() => document.getElementById('problem-area').style.animation = '', 300);
      STATE.selectedSquare = null;
      STATE.highlights = [];
      STATE.captureSquares = [];
      renderBoard(STATE.currentPuzzle);
...
function nextPuzzle() {
  // Перемешиваем и берём случайную
...
function start() {
  if (STATE.running) return;
  Object.assign(STATE, {
    timer: 300, running: true, score: 0
  });
...
function finish() {
  ...
  document.getElementById('resVs').innerHTML = `${rating}. Решено <strong>${STATE.score}</strong> тактических задач`;
