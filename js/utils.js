const Utils = {
  ...
  cursor: () => {
    ...
    const loop = () => {
      fx += (mx - fx) * 0.18;
      fy += (my - fy) * 0.18;
      f.style.transform = `translate(${fx-18}px, ${fy-18}px)`;
      requestAnimationFrame(loop);
    };
    loop();
    document.addEventListener('mouseover', e => {
      const t = e.target.closest('button, a, .b-card, .mode-btn, .ach, summary, .ctrl-btn');
      if (t) {
  ...
  saveUser: (user) => {
    try { localStorage.setItem('bp_user', JSON.stringify(user)); } catch {}
  },
  clearUser: () => {
